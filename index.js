  ...
  db(db_table_devices)
    ...
    .then((data) => {
      if (!data.length) {
        ...
        res.json({ status: 1, msg: 'Updated the database' });
      }
    })
    .catch((e) => {
      res.json({ status: 0, msg: 'Error while updating current info' });
    });
});
  ...
  db(db_table_devices)
    ...
    .then((data) => {
      if (!data.length) {
        ...
        res.json({ status: 1, msg: 'Added to the database' });
      }
    })
    .catch((e) => {
      res.json({ status: 0, msg: 'Error while adding a device' });
    });
});
...
app.post('/register_admin', async (req, res) => {
  ...
  db(db_table_employees)
    ...
    .then((data) => {
      if (!data.length) {
        res.json({ status: 0, msg: 'Unable to Add Admin' });
      } else {
        res.json({ status: 1, msg: 'Admin Added Successfully' });
      }
    })
    .catch((e) => {
      res.json({ status: 0, msg: 'Unable to Add Admin' });
...
app.post('/register_employee', async (req, res) => {
  ...
  db(db_table_employees)
    ...
    .then((data) => {
      if (!data.length) {
        res.json({ status: 0, msg: 'Unable to Add Employee' });
      } else {
        res.json({ status: 1, msg: 'Employee Added Successfully' });
      }
    })
    .catch((e) => {
      res.json({ status: 0, msg: 'Unable to Add Employee' });
...
app.post('/login', async (req, res) => {
  ...
  let foundRows = await db(db_table_employees).select('*').where({ email });

  if (foundRows.length == 0) {
    res.json({ status: 0, msg: 'No Email Found' });
    return;
  } else if (foundRows.length > 1) {
    res.json({ status: 0, msg: 'Multiple found' });
    return;
  }
  ...
  const token = await jwt.sign({ id: foundUser.id }, process.env.JWT_SECRET_KEY);
  ...
  res.send({ status: 1, role: foundUser.role });
});

app.post('/logout', (req, res) => {
  res.cookie('jwt', { maxAge: 0 });
  res.cookie('user_id', { maxAge: 0 });
  res.json({ status: 1, msg: 'Logged Out Successfully' });
});
...
app.post('/verify-user-upon-entering', isUserAuthorized, async (req, res) => {
  let userFound = await db(db_table_employees).select('*').where({ id: req.cookies['user_id'] });
  if (!userFound.length) {
    res.json({ status: 0, msg: 'Unable to find the user...' });
  }
...
app.post('/add-feedback', isUserAuthorized, async (req, res) => {
  ...
  db(db_table_feedbacks)
    ...
    .then(async (data) => {
      if (!data.length) {
        res.json({ status: 0, msg: 'Unable to add feedback' });
      } else {
        let feedbacks = await db(db_table_feedbacks).select('*');
        res.json({ status: 1, msg: 'Feedback added successfully', feedbacks });
      }
    })
    .catch((e) => {
      res.json({ status: 0, msg: 'Unable to add feedback_ERROR' });
    });
});
...
app.post('/get-unknown-devices', isUserAuthorized, async (req, res) => {
  ...
  res.json({ status: 1, unknown_devices });
});
...
app.post('/hardware-update-bin', async (req, res) => {
  ...
  if (!foundDevicesByID.length) {
    db(db_table_devices)
      ...
      .then((data) => {
        res.json({ status: 1, msg: 'Inserted into the database' });
      })
      .catch((e) => {
        res.json({ status: 0, msg: 'Error while adding the unknown device' });
      });
  } else {
    db(db_table_devices)
      .returning('*')
      ...
      .then((data) => {
        ...
        db(db_table_historical)
          ...
          .then((data) => {
            if (!data.length) {
              res.json({ status: 0, msg: 'Unable to insert into historical' });
            } else {
              res.json({ status: 1, msg: 'Updated the database' });
            }
          })
          .catch((e) => {
            console.log(e);
            res.json({ status: 0, msg: 'error caught when inserting into historical' });
          });
      })
      .catch((e) => {
        res.json({ status: 0, msg: 'Error while updating current info' });
...
app.post('/employee-register-unknown-bin', isUserAuthorized, async (req, res) => {
  const { id, lat, lng, bin_height } = req.body;

  db(db_table_devices)
    .returning('*')
    .update({ is_registered: true, lat, lng, bin_height })
    .where({ id })
    .then((data) => {
      if (!data.length) {
        res.json({ status: 0, msg: 'Was unable to register the device' });
      } else {
        res.json({ status: 1, msg: 'Device registered Successfully' });
      }
    });
});
...
app.post('/create-route', isUserAuthorized, async (req, res) => {
  ...
  db(db_table_routes)
    ...
    .then((data) => {
      if (!data.length) {
        res.json({ status: 0, msg: 'Unable to Create the route' });
      } else {
        res.json({ status: 1, msg: 'The route has been created' });
      }
    })
    .catch((e) => {
      console.log(e);
...
app.post('/start-route', isUserAuthorized, async (req, res) => {
  ...
  db(db_table_routes)
    ...
    .then((data) => {
      if (!data.length) {
        res.json({ status: 0, msg: 'Unable to start the route' });
      } else {
        res.json({ status: 1, msg: 'The route has been added' });
      }
    })
    .catch((e) => {
      console.log(e);
...
app.post('/finish-route', isUserAuthorized, async (req, res) => {
  ...
  db(db_table_routes)
    ...
    .then((data) => {
      if (!data.length) {
        res.json({ status: 0, msg: 'Unable to complete the route' });
      } else {
        res.json({ status: 1, msg: 'The route has been completed' });
      }
    })
    .catch((e) => {
      console.log(e);
...
app.post('/delete-route', isUserAuthorized, async (req, res) => {
  ...
  db(db_table_routes)
    ...
    .then((data) => {
      if (!data.length) {
        res.json({ status: 0, msg: 'Unable to delete the route' });
      } else {
        res.json({ status: 1, msg: 'The route has been deleted' });
      }
    })
    .catch((e) => {
      console.log(e);
...
app.post('/get-historical', isUserAuthorized, async (req, res) => {
  ...
  if (!startDate || !endDate) {
    ...
    const startDay = new Date(endDay.getFullYear(), endDay.getMonth(), endDay.getDate() - 28);
    db(db_table_historical)
      .select('*')
      .whereBetween('saved_time', [startDay, endDay])
      .then((data) => {
        const result = calculateWeeklyMedian(data);
        res.json({ status: 1, notManipulated: data, historicalData: result });
      });
  } else {
...
app.post('/clear-all-historical', isUserAuthorized, async (req, res) => {
  db(db_table_historical)
    ...
    .truncate()
    .then((data) => {
      res.json({ status: 1, msg: 'All historical cleared successfully' });
    })
    .catch((e) => {
      console.log(e);
      res.json({ status: 0, msg: 'Server side error occured' });
    });
});
